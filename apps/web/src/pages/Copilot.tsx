...
export const Copilot: React.FC<CopilotStudioProps> = ({ complaints = [], onSelectComplaint }) => {
  ...
  const isMunicipalStaff = user?.role !== 'citizen';

  const [activeTab, setActiveTab] = useState<'chat' | 'briefing'>('chat');
  const [briefing, setBriefing] = useState<GroundedMunicipalBriefing | null>(null);
...
Currently evaluating **${complaints.length} live municipal reports**. Click **"Generate Municipal Briefing"** above or select a quick operational inquiry below.`,
  ...
  const handleGenerateBriefing = async () => {
    ...
    try {
      ...
      const result = await CopilotService.generateMunicipalBriefing(complaints, securityContext);
      setBriefing(result);
  ...
  const handleAsk = async (text: string) => {
    ...
    if (
      ...
      text.toLowerCase().includes('municipal briefing')
    ) {
      // Also generate the dedicated briefing view
  ...
  const operationalShortcuts = [
    ...
    },
    {
      ...
      tag: 'SLA',
      action: () => handleAsk('Which complaints are overdue?'),
    },
    {
      ...
      tag: 'Ward',
      action: () => handleAsk('Summarize the situation for the roads department'),
    },
    {
      ...
      tag: 'Ward',
      action: () => handleAsk('Summarize the situation for the water supply department'),
    },
  ];

  return (
    <div className="space-y-3 max-w-7xl mx-auto h-[calc(100vh-6.5rem)] flex flex-col pb-2">
      {/* Header Banner & Mode Switcher */}
      <div className="p-3 bg-white border border-[#D9E2EC] rounded-lg shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-1.5 rounded-md bg-[#1769D2]/10 text-[#1769D2]">
              <Compass className="w-4 h-4" />
            </span>
            <h1 className="text-sm font-bold uppercase tracking-wider text-[#172B4D]">
              Municipal AI Copilot & Executive Decision Support
            </h1>
          ...
          <p className="text-xs text-[#526581] mt-0.5">
            Grounded intelligence synthesis over {complaints.length} live Supabase records.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isMunicipalStaff && (
            <Button
              ...
              className="h-8 text-xs font-semibold bg-[#1769D2] hover:bg-[#123B6D] text-white gap-1.5 shadow-xs"
            >
              <FileText className={`w-3.5 h-3.5 ${briefingLoading ? 'animate-spin' : ''}`} />
              <span>{briefingLoading ? 'Synthesizing Briefing...' : '📋 Generate Municipal Briefing'}</span>
            </Button>
          )}

          <div className="flex items-center p-0.5 bg-slate-100 border border-[#D9E2EC] rounded-lg text-xs font-mono">
            <button
              onClick={() => setActiveTab('chat')}
              className={`px-3 py-1 rounded font-bold transition-all ${
                activeTab === 'chat'
                  ? 'bg-white text-[#172B4D] shadow-2xs'
      ...
      {activeTab === 'briefing' && briefing ? (
        /* Dedicated Executive Briefing View */
        <div className="flex-1 overflow-y-auto">
          <MunicipalBriefingView
            briefing={briefing}
          ...
          <div className="lg:col-span-4 space-y-3 flex flex-col min-h-0 overflow-y-auto">
            ...
            <Card className="p-3.5 border-[#D9E2EC] bg-white shadow-sm space-y-2.5 flex-1">
              <div className="text-xs font-bold uppercase tracking-wider text-[#172B4D] flex items-center gap-1.5">
                ...
                <span>Grounded Decision Support</span>
              </div>

              <div className="space-y-2 text-xs text-[#526581]">
                <div className="p-2 rounded bg-slate-50 border border-slate-200 space-y-1">
                  <div className="text-[10px] font-bold uppercase text-[#172B4D]">Deterministic Baseline</div>
                  <div className="text-[11px]">All statistics, counts, scores, and zones originate strictly from 3A–3E engines.</div>
                </div>

                <div className="p-2 rounded bg-slate-50 border border-slate-200 space-y-1">
                  <div className="text-[10px] font-bold uppercase text-[#172B4D]">Zero Hallucination</div>
                  <div className="text-[11px]">Gemini summarizes retrieved ground-truth without fabricating numbers.</div>
                </div>

                <div className="p-2 rounded bg-slate-50 border border-slate-200 space-y-1">
                  <div className="text-[10px] font-bold uppercase text-[#172B4D]">Fallback Resilience</div>
                  <div className="text-[11px]">Deterministic generator activates automatically if AI endpoint is offline.</div>
                </div>
              </div>
            </Card>
          </div>
