...
export class GroundingSecurityGuard {
  ...
  private static readonly injectionPatterns = [
    ...
    /pretend\s+(i\s+am|you\s+are)\s+(super_admin|municipal_admin|admin|officer|staff)/i,
    /bypass\s+(rls|security|authorization|roles|policies)/i,
    /ignore\s+(rls|security|authorization|roles|policies)/i,
    /show\s+all\s+(database\s+)?(complaints|reports|records|profiles|users)/i,
    /give\s+me\s+(private\s+)?(citizen|user|personal)\s+(data|info|records|aadhaar|phone)/i,
    /drop\s+table/i,
